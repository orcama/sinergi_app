...
def library_save(body: LibrarySaveRequest, user: dict) -> LibraryItem:
    ...
    blob.make_public()
    db.collection("files").document(file_id).set({"user_id": user["uid"], "name": body.name, "size": body.size, "text": body.text, "token_count": body.token_count, "chat_id": body.chat_id or "", "storage_path": path, "type": "document", "extension": ext.lstrip(".") or "pdf", "created_at": firestore.SERVER_TIMESTAMP})
...
def library_list(user: dict) -> JSONResponse:
    items = []
    for doc in db.collection("files").where("user_id", "==", user["uid"]).stream():
        data = doc.to_dict()
        created = data.get("created_at")
        items.append({"id": doc.id, "name": data.get("name", ""), "type": data.get("type", "document"), "extension": data.get("extension", "pdf"), "modified_at": created.isoformat() if created else data.get("modified_at", ""), "size_in_bytes": data.get("size", 0), "chat_id": data.get("chat_id") or None, "storage_path": data.get("storage_path", ""), "token_count": data.get("token_count", 0)})
