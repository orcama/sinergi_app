...
async def chat_session_list_route(user: dict = Depends(require_verified_user)):
    return chat_session_list(user)


@router.post("/api/chats", response_model=ChatSessionItem)
async def chat_session_save_route(body: ChatSessionSaveRequest, user: dict = Depends(require_verified_user)):
    return chat_session_save(body, user)


@router.get("/api/chats/{chat_id}", response_model=ChatSessionItem)
async def chat_session_get_route(chat_id: str, user: dict = Depends(require_verified_user)):
    return chat_session_get(chat_id, user)


@router.delete("/api/chats/{chat_id}")
async def chat_session_delete_route(chat_id: str, user: dict = Depends(require_verified_user)):
